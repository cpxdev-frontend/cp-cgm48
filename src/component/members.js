...
const Memberlist = ({ fet, setSec, width }) => {
      ...
      fetch(
        ...
        .then(async (data) => {
          ...
          if (search !== "") {
            ...
            setSearch(txt);
            const d = data.response.filter((x) =>
  ...
  React.useEffect(() => {
    if (
      seGrad == 2 &&
      _DATA.currentData().filter((x) => x.graduated == true).length > 0
    ) {
      count = Math.ceil(
        _DATA.currentData().filter((x) => x.graduated == true).length / PER_PAGE
      );
    } else if (
      seGrad == 1 &&
      _DATA.currentData().filter((x) => x.graduated == false).length > 0
    ) {
      count = Math.ceil(
        _DATA.currentData().filter((x) => x.graduated == false).length /
          PER_PAGE
      );
    }
    if (pageset > 1) {
    ...
    <>
      ...
      <div className="stage text-center pt-5 pb-2">
        {Loaded && Arr.length > 0 && (
          <Card
            className={
              "text-left " + (width > 700 ? "ml-5 mr-5" : "ml-2 mr-2")
            }>
            <TextField
              label="Search Member"
              value={search}
              className="m-3"
              onChange={(e) => SearchEng(e.target.value)}
            />
            <TextField
              select
              label="Choose Group"
        ...
        <Zoom in={mem.length > 0 ? Loaded : false}>
          <Card className="mt-2 ml-5 mr-5">
            <CardContent>
              Found{" "}
              {seGrad == 2
                ? mem.filter((x) => x.graduated == true).length
                : seGrad == 1
